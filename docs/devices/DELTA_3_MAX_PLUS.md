...
- Solar Input Power (`pow_get_h`)
- Solar Input Voltage (`pow_get_h_vol`)
- Solar Input Current (`pow_get_h_amp`)
- USB QC (1) Out Power (`pow_get_qcusb1`)
- Type-C (1) Out Power (`pow_get_typec1`)
- Type-C (2) Out Power (`pow_get_typec2`)
- Type-C (3) Out Power (`pow_get_typec3`)
...
- AC Output Enabled (`cfg_ac_out_open` -> `{"moduleType": 0, "operateType": "TCP", "params": {"id": 66, "cfgAcOutOpen": "VALUE"}}`)
